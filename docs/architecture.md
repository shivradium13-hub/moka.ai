...
| Crawler politeness | robots-parser | MIT |
...
```
Question → Planner → Search → Collect → Extract → Verify → Synthesize → Cite
```

All egress goes through `safeFetch` (SSRF-guarded). Citations are captured at fetch time from genuinely retrieved documents — **never generated by the model** (§9, §45).
